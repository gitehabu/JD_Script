...
async function jdFruit() {
    ...
    if ($.farmInfo.farmUserPro) {
        ...
        console.log(`\n当前账号【${$.nickName || $.UserName}】的shareCode：${$.farmInfo.farmUserPro.shareCode} ，已成功兑换${$.farmInfo.farmUserPro.winTimes}次水果`);
        ...
        await getExtraAward();          // 获取助力信息, 领取额外水滴奖励
        // await shareCodesFormat();       // 助力列表
        await masterHelpShare();        // 助力好友

        await doDailyTask();            // 签到
...
async function getExtraAward() {
    ...
    if ($.masterHelpResult.code === '0') {
        ...
        if ($.masterHelpResult.masterHelpPeoples && $.masterHelpResult.masterHelpPeoples.length > 0) {
            let str = '';
            helpFriends = $.masterHelpResult.masterHelpPeoples.map((item, index) => {
                ...
                let time = ((date.getHours() === 23) ? 7 : (date.getHours() + 8)) + ':' + date.getMinutes() + ':' + date.getSeconds();
...
function shareCodesFormat() {
    // console.log("--------------------------------------------------------")
    // console.log($.masterHelpResult.masterHelpPeoples)
    // console.log(helpFriends)
    const sCode = $.isNode() ? require('./jdFruitShareCodes.js') : '';
    jdFruitShareArr = sCode.hy(helpFriends, $.nickName, $.farmInfo.farmUserPro.shareCode)
    // console.log('001', arr)
...
async function masterHelpShare() {
    const sCode = $.isNode() ? require('./jdFruitShareCodes.js') : '';
    newShareCodes = sCode.hy(helpFriends, $.nickName, $.farmInfo.farmUserPro.shareCode) || []
    ...
    for (let code of newShareCodes) {
        ...
        if ($.helpResult.code === '0') {
            let name = $.helpResult.helpResult.masterUserInfo.nickName
            if ($.helpResult.helpResult.code === '0') {
                //助力成功
                salveHelpAddWater += $.helpResult.helpResult.salveHelpAddWater;
                console.log(`✔ 为【${name}】助力成功, 获得${$.helpResult.helpResult.salveHelpAddWater}g水滴`);
                helpSuccessPeoples += (name || '匿名用户') + ',';
            } else if ($.helpResult.helpResult.code === '8') {
                console.log(`❌ 为 ${name} 助力失败，今日已无助力机会`);
            } else if ($.helpResult.helpResult.code === '9') {
                console.log(`❌ 好友 ${name} 已助力過`);
            } else if ($.helpResult.helpResult.code === '10') {
                console.log(`❌ 好友 ${name} 已满5人助力`);
            } else {
                console.log(`❌ 助力其他情况：${JSON.stringifsy($.helpResult.helpResult)}`);
            }
            remainTimes = $.helpResult.helpResult.remainTimes;
            console.log(`【今日助力次数余】${remainTimes}次\n`);
            if (remainTimes === 0) {
                console.log(`❌ 今日助力次數用完`);
                break
            }
        } else {
...
async function doFriendsWater() {
    ...
    if ($.friendList.friends && $.friendList.friends.length > 0) {
        ...
        console.log(`浇水列表: ${JSON.stringify(needWaterFriends)}`);

        let waterFriendsCount = 0, cardInfoStr = '';
        for (let index = 0; index < needWaterFriends.length; index ++) {
            let friend = await request('friendInitForFarm', {"shareCode": needWaterFriends[index].shareCode, "version": 6, "channel": 1})
            if (friend.code != 0) {
                console.log('❌ 获取不到好友信息')
                continue;
            }

            console.log(`为第 ${index+1} 个好友 ${(needWaterFriends[index].name || needWaterFriends[index].nickName)} 浇水...`)
            if (!friend.hadWateredFriend && friend.canWaterFriend) {
                await waterFriendForFarm(needWaterFriends[index].shareCode);
                ...
                console.log(`浇水结果: ${JSON.stringify($.waterFriendForFarmRes)}`)
            } else if (friend.hadWateredFriend) {
                console.log('✔ 已为好友 '+ (needWaterFriends[index].name || needWaterFriends[index].nickName) +' 浇过, 明天在来吧~ \n')
            } else if (!friend.canWaterFriend) {
                console.log('✔ 好多人帮忙浇水, 去帮帮别人吧~\n')
            } else {
                console.log("❌ Error 浇水失败\n")
            }
        }
        // message += `【好友浇水】已给${waterFriendsCount}个好友浇水,消耗${waterFriendsCount * 10}g水\n`;
        console.log(`==> 【好友浇水】当前给${waterFriendsCount}个好友浇水,消耗${waterFriendsCount * 10}g水\n\n`);
