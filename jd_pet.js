...
!(async () => {
    ...
    for (let i = 0; i < cookiesArr.length; i++) {
        if (cookiesArr[i]) {
            ...
            $.nickName = '';
            await TotalBean();
            console.log(`🐶🐶🐶🐶🐶🐶🐶🐶🐶🐶🐶🐶🐶🐶🐶🐶🐶🐶🐶🐶🐶🐶🐶🐶🐶🐶🐶 开始账号 ${$.index}【${$.nickName || $.UserName}】 🐶🐶🐶🐶🐶🐶🐶🐶🐶🐶🐶🐶🐶🐶🐶🐶🐶🐶🐶🐶🐶🐶🐶🐶🐶🐶🐶\n`);
            if (!$.isLogin) {
...
async function jdPet() {
    ...
    if (initPetTownRes.code === '0' && initPetTownRes.resultCode === '0' && initPetTownRes.message === 'success') {
        ...
        console.log(`\n当前账号【${$.nickName || $.UserName}】的shareCode： ${$.petInfo.shareCode}\n`);
        await taskInit();
    ...
    if (res.code === '0' && res.resultCode === '0') {
        ...
        if (res.result.masterHelpPeoples && res.result.masterHelpPeoples.length >= 5) {
            if(!res.result.addedBonusFlag) {
                ...
                console.log(`✔ 领取成功，奖励结果：【${getHelpAddedBonusResult.message}】`);
                message += `【助力满员奖励 ${getHelpAddedBonusResult.result.reward} 领取】 ${getHelpAddedBonusResult.message}\n`;
            } else {
                console.log("❌ 领取失败，原因：已领过");
                message += `【助力满员奖励】 已领取\n`;
            }
        } else {
            console.log("❌ 领取失败，原因：未达满员")
            message += `【助力满员奖励】 领取失败，原因：未达满员\n`;
        }
    }
}
// 助力列表
    ...
    for (let code of newShareCodes) {
        ...
        if (response.code === '0' && response.resultCode === '0') {
            if (response.result.helpStatus === 0) {
                console.log('✔ 为【' + response.result.masterNickName + '】助力成功');
                helpPeoples += response.result.masterNickName + '，';
            } else if (response.result.helpStatus === 1) {
                // 您今日已无助力机会
                console.log(`❌ 为 ${response.result.masterNickName} 助力失败，今日已无助力机会`);
                break;
            } else if (response.result.helpStatus === 2) {
                //该好友已满5人助力，无需您再次助力
                console.log(`❌ 好友 ${response.result.masterNickName} 已满5人助力`);
            } else {
...
function requireConfig() {
    return new Promise(resolve => {
        ...
        //Node.js用户请在jdCookie.js处填写京东Cookie;
